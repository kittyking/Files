...
//  *****************只有把添加消息的操作放在handler里 刷新才能显示*****************
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(username.equals("shop"))
			this.titleTextView.setText(getString(R.string.name_dddt));
		else
			this.titleTextView.setText(username);
		getScreenHeighAndWidth();
		
		chatListView = (ListView)findViewById(R.id.chatInfo);
		adapter = new ChatDetailAdapter(this, LocalSharedPreferenceSingleton.getInstance().getUserInfo(this).getData().getUserName());
		...
		chatET.setOnEditorActionListener(new OnEditorActionListener() {
			...
			public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
				...
				if(actionId==EditorInfo.IME_ACTION_SEND){
					...
					message.setAttribute("nick", LocalSharedPreferenceSingleton.getInstance().getUserInfo(ChatDetailActivity.this).getData().getNickName());
					message.setAttribute("avatar", LocalSharedPreferenceSingleton.getInstance().getUserInfo(ChatDetailActivity.this).getData().getAvatar());
					SimpleDateFormat    sDateFormat    =   new    SimpleDateFormat("MM-dd hh:mm:ss");       
					String    date    =    sDateFormat.format(new    java.util.Date());   
					message.setAttribute("time", date);
					message.setFrom(LocalSharedPreferenceSingleton.getInstance().getUserInfo(ChatDetailActivity.this).getData().getMobile());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode ==1001){//从相册得到图片
			...
			String path;
			path=ImageHandlerSingleton.getInstance().getImageAbsolutePath(this, uri);
//			if(ImageHandlerSingleton.getSDKVersionNumber()>=4.4){
//				//4.4以上版本获取本地路径
//				path=ImageHandlerSingleton.getInstance().getImageAbsolutePath(this, uri);
//			}else{
//				//获得本地图片路径
				...
				for(int col =0;col<7;col++){
					...
					ib.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							Log.i("", "htmlStr="+htmlStr);
							CharSequence text= ChatEmojiClass.getInstance().setTxtByHtmlStrAndPosition(htmlStr, ChatDetailActivity.this, picIndex);
							inputStr="";
							isEmoji=1;
							chatET.setText(text);
						}
